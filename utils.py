...
def choose_line(file_name: str, length: int = gs.WORDLENGTH):
	# do we have an absolute path or a relative one?\
	...
	with open(file_name, 'r') as file: # you need UTF16 when file was written by Powershell versions other than Core
		lines = file.readlines()
	if length > 0: 
		lines = [x for x in lines if len(x) == length] # filter the list for lines with length
	if len(lines) == 0:
		raise ValueError(f"No words with length {length} found.")
	random_word = random.choice(lines).rstrip().lower() # if you don't strip you may be left with a dangling \r on Windows
	return random_word 
...
def evaluate_answer_state(answer, hint, guess):
	# no input
	if not guess:
...
def replace_character_in_same_sized_string(answer, hint, char):
